...
<section role="main" class="content-body">
   ...
   <div class="row mb-2">
        <div class="col">
            <section class="card">
                <header class="card-header bg-theme">
                    ...
                    <h2 class="card-title color-theme text-center">Select Client</h2>
                </header>
                <div class="card-body">
                    <form action="{{route('favourite_list_individual')}}" method="post">
                        @csrf
                            ...
                            <div class="col-lg-6 m-auto">
                                <select name="client_id" data-plugin-selectTwo class="form-control populate" onchange="this.form.submit()">
                                    ...
                                    @foreach($users as $key => $user)
                                        <option @if(isset($client_id) && $user->id==$client_id) selected @endif  value="{{$user->id}}"> {{$user->name}} </option>
                                    @endforeach
                                    @endif
    ...
    @if(isset($client_id) && isset($favourites) && count($favourites)>0)
      ...
      <div class="col-lg-2-5 col-xl-1-5 mb-4 mb-lg-0">
         <div class="filters-sidebar-wrapper bg-light rounded">
            <div class="card card-modern">
               ...
               <?php $product=DB::table('products')->where('id',$favourite->product_id)->first();?>
               ...
               <div class="card-body">
                  <ul class="list list-unstyled mb-0 controls">
                     <li><a type="button" class="control" data-filter="*">All</a></li>
         ...
         <div class="col-lg-3-5 col-xl-4-5">
            ...
            @if(isset($favourites) && count($favourites)>0)
            <form action="{{url('delete_saved_product')}}/{{$client_id}}" method="post">
            @csrf
            <div id="products_container" class="row row-gutter-sm">
                @foreach($favourites as $key => $favourite)
                <?php $product=DB::table('products')->where('id',$favourite->product_id)->first();?>
                    <div class="col-sm-6 col-xl-3 mb-2 px-1 product-item {{$product->category}}">
                        <li class="list-group-item d-flex justify-content-between lh-condensed">
                            ...
                            <input  id="prova{{$key}}" type="checkbox" name="product_id[]" value="{{$favourite->id}}">
                        </li>
                    </div>
